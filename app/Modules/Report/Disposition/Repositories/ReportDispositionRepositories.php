...
namespace App\Modules\Report\Disposition\Repositories;
...
class ReportDispositionRepositories extends BaseRepository implements ReportDispositionInterface
{
    ...
    {
        $user_structure_id = Auth::user()->user_core ? Auth::user()->user_core->kode_struktur : 0;
        ...
        ])->whereHas('assign', function ($q) use ($user_structure_id) {
            $q->whereHas('structure', function ($as) use ($user_structure_id) {
                $as->where('structure_id', $user_structure_id);
            });
        })
