...
class AuthRepositories extends BaseRepository implements AuthInterface
{
	...
	public function processAuth($request)
	{
		try {
			...
			// dd($token);
			// $refresh_token = $user->createToken('Token Name')->refreshToken;
			// dd($refresh_token);
			// $c = HttpClient::post('http://localhost:8001/api/v1/oauth/token');
			// $c->jsonBody($data);
			// $res = $c->call();
			// dd($res);
			$response = $guzzle->post('http://localhost/api/v1/oauth/token', [
				'form_params' => [
					'grant_type' => 'password',
					'client_id' => $client_id,
					'client_secret' => $client_secret,
